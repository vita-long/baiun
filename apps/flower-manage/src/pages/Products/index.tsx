...
const Products: React.FC = () => {
  ...
  const { data: products, run: fetchProduct, loading } = useRequest(() => getProducts({
    page,
    pageSize,
    productType: ProductTypeMap.Normal
  }), { manual: true });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (isNew: boolean) => (
        <Switch checked={isNew || false} disabled />
      ),
    },
    {
      title: '是否推荐',
      ...
      render: (isRecommend: boolean) => (
        <Switch checked={isRecommend || false} disabled />
      ),
    },
    {
      title: '是否上架',
        ...
        >
          ...
          >
            <Select placeholder="请选择商品类型">
              ...
              <Select.Option value="points">积分商品</Select.Option>
            </Select>
          </Form.Item>
